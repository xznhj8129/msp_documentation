...
#define MSP_OSD_VIDEO_CONFIG_ID 180

#define MSP_SET_OSD_VIDEO_CONFIG_ID 181

#define MSP_DISPLAYPORT_ID 182
...
#define MSP_RESERVE_1_ID 251

#define MSP_RESERVE_2_ID 252
...
#define MSP2_COMMON_GET_RADAR_GPS_ID 4111
#define MSP2_COMMON_GET_RADAR_GPS_MSPV 2
...
typedef struct MSP_PACKED {
    ...
    uint8_t commCapabilities; // Bitmask: Communication capabilities: Bit 0=VCP support (`USE_VCP`), Bit 1=SoftSerial support (`USE_SOFTSERIAL1`/`2`)
...
#define MSP_BUILD_INFO_REPLY_PAYLOAD_SIZE 27
...
typedef struct MSP_PACKED {
    ...
    uint8_t accLpfHz; // Accelerometer LPF frequency (`accelerometerConfig()->acc_lpf_hz`) cutoff frequency for the low pass filter used on the acc z-axis for althold in Hz
...
typedef struct MSP_PACKED {
    ...
    navMcAltHoldThrottle_e mcAltHoldThrottleType; // Enum `navMcAltHoldThrottle_e` mirrored from `navConfig()->mc.althold_throttle_type`.
    uint16_t mcHoverThrottle; // Multirotor hover throttle PWM value (`currentBatteryProfile->nav.mc.hover_throttle`).
...
typedef struct MSP_PACKED {
    navUserControlMode_e userControlMode; // Sets `navConfigMutable()->general.flags.user_control_mode`.
    uint16_t maxAutoSpeed; // Sets `navConfigMutable()->general.max_auto_speed`.
    uint16_t maxAutoClimbRate; // Sets `navConfigMutable()->fw.max_auto_climb_rate` or `navConfigMutable()->mc.max_auto_climb_rate` based on `mixerConfig()->platformType`.
    uint16_t maxManualSpeed; // Sets `navConfigMutable()->general.max_manual_speed`.
    uint16_t maxManualClimbRate; // Sets `navConfigMutable()->fw.max_manual_climb_rate` or `navConfigMutable()->mc.max_manual_climb_rate`.
    uint8_t mcMaxBankAngle; // Sets `navConfigMutable()->mc.max_bank_angle`.
    navMcAltHoldThrottle_e mcAltHoldThrottleType; // Enum `navMcAltHoldThrottle_e`; updates `navConfigMutable()->mc.althold_throttle_type`.
...
typedef struct MSP_PACKED {
    uint8_t accCalibAxisFlags; // Bitmask: Flags indicating which axes of the accelerometer have been calibrated (`accGetCalibrationAxisFlags()`)
    int16_t accZeroX; // Accelerometer zero offset for X-axis (`accelerometerConfig()->accZero.raw[X]`)
    int16_t accZeroY; // Accelerometer zero offset for Y-axis (`accelerometerConfig()->accZero.raw[Y]`)
    int16_t accZeroZ; // Accelerometer zero offset for Z-axis (`accelerometerConfig()->accZero.raw[Z]`)
    int16_t accGainX; // Accelerometer gain/scale for X-axis (`accelerometerConfig()->accGain.raw[X]`)
    int16_t accGainY; // Accelerometer gain/scale for Y-axis (`accelerometerConfig()->accGain.raw[Y]`)
    int16_t accGainZ; // Accelerometer gain/scale for Z-axis (`accelerometerConfig()->accGain.raw[Z]`)
    int16_t magZeroX; // Magnetometer zero offset for X-axis (`compassConfig()->magZero.raw[X]`). 0 if `USE_MAG` disabled
    int16_t magZeroY; // Magnetometer zero offset for Y-axis (`compassConfig()->magZero.raw[Y]`). 0 if `USE_MAG` disabled
    int16_t magZeroZ; // Magnetometer zero offset for Z-axis (`compassConfig()->magZero.raw[Z]`). 0 if `USE_MAG` disabled
    uint16_t opflowScale; // Optical flow scale factor (`opticalFlowConfig()->opflow_scale * 256`). 0 if `USE_OPFLOW` disabled
    int16_t magGainX; // Magnetometer gain/scale for X-axis (`compassConfig()->magGain[X]`). 0 if `USE_MAG` disabled
    int16_t magGainY; // Magnetometer gain/scale for Y-axis (`compassConfig()->magGain[Y]`). 0 if `USE_MAG` disabled
    int16_t magGainZ; // Magnetometer gain/scale for Z-axis (`compassConfig()->magGain[Z]`). 0 if `USE_MAG` disabled
...
typedef struct MSP_PACKED {
    int16_t accZeroX; // Sets `accelerometerConfigMutable()->accZero.raw[X]`.
    int16_t accZeroY; // Sets `accelerometerConfigMutable()->accZero.raw[Y]`.
    int16_t accZeroZ; // Sets `accelerometerConfigMutable()->accZero.raw[Z]`.
    int16_t accGainX; // Sets `accelerometerConfigMutable()->accGain.raw[X]`.
    int16_t accGainY; // Sets `accelerometerConfigMutable()->accGain.raw[Y]`.
    int16_t accGainZ; // Sets `accelerometerConfigMutable()->accGain.raw[Z]`.
    int16_t magZeroX; // Sets `compassConfigMutable()->magZero.raw[X]` (if `USE_MAG`)
    int16_t magZeroY; // Sets `compassConfigMutable()->magZero.raw[Y]` (if `USE_MAG`)
    int16_t magZeroZ; // Sets `compassConfigMutable()->magZero.raw[Z]` (if `USE_MAG`)
    uint16_t opflowScale; // Sets `opticalFlowConfigMutable()->opflow_scale = value / 256.0f` (if `USE_OPFLOW`)
    int16_t magGainX; // Sets `compassConfigMutable()->magGain[X]` (if `USE_MAG`)
    int16_t magGainY; // Sets `compassConfigMutable()->magGain[Y]` (if `USE_MAG`)
    int16_t magGainZ; // Sets `compassConfigMutable()->magGain[Z]` (if `USE_MAG`)
...
typedef struct MSP_PACKED {
    ...
    uint8_t rthTailFirst; // Flag: Return tail-first during RTH (`navConfig()->general.flags.rth_tail_first`)
    uint8_t rthAllowLanding; // Flag: Allow automatic landing after RTH (`navConfig()->general.flags.rth_allow_landing`)
    navRthAltMode_e rthAltControlMode; // RTH altitude control mode (`navConfig()->general.flags.rth_alt_control_mode`).
    uint16_t rthAbortThreshold; // Distance increase threshold to abort RTH (`navConfig()->general.rth_abort_threshold`)
...
typedef struct MSP_PACKED {
    ...
    navRthAltMode_e rthAltControlMode; // Sets `navConfigMutable()->general.flags.rth_alt_control_mode`.
...
typedef struct MSP_PACKED {
    uint16_t cruiseThrottle; // Cruise throttle command (`currentBatteryProfile->nav.fw.cruise_throttle`).
    ...
    uint8_t pitchToThrottle; // Pitch-to-throttle gain (`currentBatteryProfile->nav.fw.pitch_to_throttle`); PWM microseconds per degree (10 units ≈ 1% throttle).
...
typedef struct MSP_PACKED {
    ...
    uint8_t pitchToThrottle; // Sets `currentBatteryProfileMutable->nav.fw.pitch_to_throttle` (PWM microseconds per degree; 10 units ≈ 1% throttle).
...
typedef struct MSP_PACKED {
    ...
    uint8_t rangeStartStep; // Start step (0-48). Each step is 25 PWM units; 0 is <=900 and 48 is >=2100.
    uint8_t rangeEndStep; // End step (0-48). Uses the same 25-PWM step mapping as rangeStartStep.
...
typedef struct MSP_PACKED {
    uint32_t featureMask; // Bitmask: active features (see `featureMask()`)
...
typedef struct MSP_PACKED {
    uint32_t featureMask; // Bitmask: features to enable
...
typedef struct MSP_PACKED {
    int16_t rollAlign; // Board alignment roll angle (`boardAlignment()->rollDeciDegrees`). Negative values tilt left.
    int16_t pitchAlign; // Board alignment pitch angle (`boardAlignment()->pitchDeciDegrees`). Negative values nose down.
    int16_t yawAlign; // Board alignment yaw angle (`boardAlignment()->yawDeciDegrees`). Negative values rotate counter-clockwise.
...
typedef struct MSP_PACKED {
    int16_t rollAlign; // Sets `boardAlignmentMutable()->rollDeciDegrees`.
    int16_t pitchAlign; // Sets `boardAlignmentMutable()->pitchDeciDegrees`.
    int16_t yawAlign; // Sets `boardAlignmentMutable()->yawDeciDegrees`.
...
typedef struct MSP_PACKED {
    int16_t scale; // Current sensor scale factor (`batteryMetersConfig()->current.scale`). Stored in 0.1 mV/A; signed for calibration.
    int16_t offset; // Current sensor offset (`batteryMetersConfig()->current.offset`). Signed millivolt adjustment.
    currentSensor_e type; // Enum `currentSensor_e` Type of current sensor hardware
...
typedef struct MSP_PACKED {
    int16_t scale; // Sets `batteryMetersConfigMutable()->current.scale` (0.1 mV/A, signed).
    int16_t offset; // Sets `batteryMetersConfigMutable()->current.offset` (signed millivolts).
    currentSensor_e type; // Enum `currentSensor_e` Sets `batteryMetersConfigMutable()->current.type`.
...
typedef struct MSP_PACKED {
    rxSerialReceiverType_e serialRxProvider; // Enum `rxSerialReceiverType_e`. Serial RX provider (`rxConfig()->serialrx_provider`).
    ...
    rxReceiverType_e receiverType; // Enum `rxReceiverType_e` Receiver type (Parallel PWM, PPM, Serial) ('rxConfig()->receiverType')
...
typedef struct MSP_PACKED {
    rxSerialReceiverType_e serialRxProvider; // Enum `rxSerialReceiverType_e`. Sets `rxConfigMutable()->serialrx_provider`.
    ...
    rxReceiverType_e receiverType; // Enum `rxReceiverType_e` Sets `rxConfigMutable()->receiverType`.
...
typedef struct MSP_PACKED {
    ...
    uint8_t rangeStartStep; // Start step (0-48). Each step is 25 PWM units; 0 is <=900 and 48 is >=2100.
    uint8_t rangeEndStep; // End step (0-48). Uses the same 25-PWM step mapping as rangeStartStep.
    adjustmentFunction_e adjustmentFunction; // Function/parameter being adjusted (see `adjustmentFunction_e`).
...
typedef struct MSP_PACKED {
    ...
    uint8_t rangeStartStep; // Start step (0-48). Each step is 25 PWM units; 0 is <=900 and 48 is >=2100.
    uint8_t rangeEndStep; // End step (0-48). Uses the same 25-PWM step mapping as rangeStartStep.
    adjustmentFunction_e adjustmentFunction; // Function/parameter being adjusted.
...
typedef struct MSP_PACKED {
    int32_t rangefinderAltitude; // Latest altitude reading from the rangefinder (`rangefinderGetLatestAltitude()`). 0 if `USE_RANGEFINDER` disabled or no reading.
...
typedef struct MSP_PACKED {
    ...
    uint16_t failsafeThrottleLowDelay; // Delay for throttle-based failsafe detection (`failsafeConfig()->failsafe_throttle_low_delay`). Units of 0.1 seconds.
    failsafeProcedure_e failsafeProcedure; // Enum `failsafeProcedure_e` Failsafe procedure (Drop, RTH, Land, etc.) ('failsafeConfig()->failsafe_procedure')
    uint8_t failsafeRecoveryDelay; // Delay after RTH finishes before attempting recovery (`failsafeConfig()->failsafe_recovery_delay`)
    int16_t failsafeFWRollAngle; // Fixed-wing failsafe roll angle (`failsafeConfig()->failsafe_fw_roll_angle`). Signed deci-degrees.
    int16_t failsafeFWPitchAngle; // Fixed-wing failsafe pitch angle (`failsafeConfig()->failsafe_fw_pitch_angle`). Signed deci-degrees.
    int16_t failsafeFWYawRate; // Fixed-wing failsafe yaw rate (`failsafeConfig()->failsafe_fw_yaw_rate`). Signed degrees per second.
    uint16_t failsafeStickThreshold; // Stick movement threshold to exit failsafe (`failsafeConfig()->failsafe_stick_motion_threshold`)
    uint16_t failsafeMinDistance; // Minimum distance from home for RTH failsafe (`failsafeConfig()->failsafe_min_distance`). Units of centimeters.
    failsafeProcedure_e failsafeMinDistanceProc; // Enum `failsafeProcedure_e` Failsafe procedure if below min distance ('failsafeConfig()->failsafe_min_distance_procedure')
...
typedef struct MSP_PACKED {
    ...
    uint16_t failsafeThrottleLowDelay; // Sets `failsafeConfigMutable()->failsafe_throttle_low_delay`. Units of 0.1 seconds.
    failsafeProcedure_e failsafeProcedure; // Enum `failsafeProcedure_e`. Sets `failsafeConfigMutable()->failsafe_procedure`.
    uint8_t failsafeRecoveryDelay; // Sets `failsafeConfigMutable()->failsafe_recovery_delay`.
    int16_t failsafeFWRollAngle; // Sets `failsafeConfigMutable()->failsafe_fw_roll_angle`. Signed deci-degrees.
    int16_t failsafeFWPitchAngle; // Sets `failsafeConfigMutable()->failsafe_fw_pitch_angle`. Signed deci-degrees.
    int16_t failsafeFWYawRate; // Sets `failsafeConfigMutable()->failsafe_fw_yaw_rate`. Signed degrees per second.
    uint16_t failsafeStickThreshold; // Sets `failsafeConfigMutable()->failsafe_stick_motion_threshold`.
    uint16_t failsafeMinDistance; // Sets `failsafeConfigMutable()->failsafe_min_distance`. Units of centimeters.
    failsafeProcedure_e failsafeMinDistanceProc; // Enum `failsafeProcedure_e`. Sets `failsafeConfigMutable()->failsafe_min_distance_procedure`.
...
typedef struct MSP_PACKED {
    osdDriver_e osdDriverType; // Enum `osdDriver_e`: `OSD_DRIVER_MAX7456` if `USE_OSD`, else `OSD_DRIVER_NONE`.
    videoSystem_e videoSystem; // Enum `videoSystem_e`: Video system (Auto/PAL/NTSC) (`osdConfig()->video_system`). Sent even if OSD disabled
    osd_unit_e units; // Enum `osd_unit_e` Measurement units (Metric/Imperial) (`osdConfig()->units`). Sent even if OSD disabled
    uint8_t rssiAlarm; // RSSI alarm threshold (`osdConfig()->rssi_alarm`). Sent even if OSD disabled
    uint16_t capAlarm; // Capacity alarm threshold (`currentBatteryProfile->capacity.warning`). Truncated to 16 bits. Sent even if OSD disabled.
    uint16_t timerAlarm; // Timer alarm threshold in minutes (`osdConfig()->time_alarm`). Sent even if OSD disabled.
...
// No request payload for MSP_OSD_CHAR_WRITE.
...
typedef struct MSP_PACKED {
    ...
    vtxLowerPowerDisarm_e lowPowerDisarm; // Enum `vtxLowerPowerDisarm_e`: Low-power behaviour while disarmed (`vtxSettingsConfig()->lowPowerDisarm`).
...
typedef struct MSP_PACKED {
    ...
    motorPwmProtocolTypes_e motorPwmProtocol; // Motor PWM protocol type (`motorConfig()->motorPwmProtocol`).
    uint16_t motorPwmRate; // Motor PWM rate (if applicable) (`motorConfig()->motorPwmRate`).
    uint16_t servoPwmRate; // Servo PWM rate (`servoConfig()->servoPwmRate`).
...
typedef struct MSP_PACKED {
    ...
    motorPwmProtocolTypes_e motorPwmProtocol; // Sets `motorConfigMutable()->motorPwmProtocol`.
...
typedef struct MSP_PACKED {
    ...
    uint32_t Platform_Capability; // Bitmask: MW capabilities
...
typedef struct MSP_PACKED {
    ...
    uint16_t sensorStatus; // Bitmask: available/active sensors (`packSensorStatus()`). See notes
    uint32_t activeModesLow; // Bitmask: First 32 bits of the active flight modes bitmask (`packBoxModeFlags()`)
...
typedef struct MSP_PACKED {
    int16_t[8] motorOutputs; // Array of current motor output values (typically 1000-2000). Values beyond `MAX_SUPPORTED_MOTORS` are 0
...
typedef struct MSP_PACKED {
    int16_t[] rcChannels; // Array of current RC channel values (typically 1000-2000). Length depends on detected channels
} msp_MSP_RC_reply_t;
...
typedef struct MSP_PACKED {
    gpsFixType_e fixType; // Enum `gpsFixType_e` GPS fix type (`gpsSol.fixType`)
    ...
    int16_t altitude; // Altitude above MSL (`gpsSol.llh.alt`) sent as centimeters
    int16_t speed; // Ground speed (`gpsSol.groundSpeed`)
    int16_t groundCourse; // Ground course (`gpsSol.groundCourse`)
...
typedef struct MSP_PACKED {
    ...
    int16_t directionToHome; // Direction to the home point (0-360) (`GPS_directionToHome`)
...
typedef struct MSP_PACKED {
    ...
    uint16_t yaw; // Yaw/Heading angle (`attitude.values.yaw`)
...
typedef struct MSP_PACKED {
    boxBitmask_t activeModes; // Bitmask: all active modes (`packBoxModeFlags()`). Size depends on `boxBitmask_t` definition
...
typedef struct MSP_PACKED {
    ...
    gpsProvider_e gpsType; // Enum `gpsProvider_e` GPS provider type (`gpsConfig()->provider`). 0 if `USE_GPS` disabled
    uint8_t legacyGpsBaud; // Always 0 (Legacy)
    sbasMode_e gpsSbasMode; // Enum `sbasMode_e` GPS SBAS mode (`gpsConfig()->sbasMode`). 0 if `USE_GPS` disabled
...
typedef struct MSP_PACKED {
    ...
    navWaypointActions_e action; // Enum `navWaypointActions_e` Waypoint action type
    ...
    int16_t param1; // Parameter 1 (meaning depends on action)
    int16_t param2; // Parameter 2 (meaning depends on action)
    int16_t param3; // Parameter 3 (meaning depends on action)
    uint8_t flag; // Bitmask: Waypoint flags (`NAV_WP_FLAG_*`)
} msp_MSP_WP_reply_t;
...
typedef struct MSP_PACKED {
    int16_t min; // Minimum servo endpoint (`servoParams(i)->min`)
    int16_t max; // Maximum servo endpoint (`servoParams(i)->max`)
    int16_t middle; // Middle/Neutral servo position (`servoParams(i)->middle`)
    int8_t rate; // Servo rate/scaling (`servoParams(i)->rate`, -125..125). Encoded as two's complement
    uint8_t reserved1; // Always 0
...
typedef struct MSP_PACKED {
    navSystemStatus_Mode_e navMode; // Enum (`navSystemStatus_Mode_e`): Current navigation mode (None, RTH, NAV, Hold, etc.) (`NAV_Status.mode`)
    navSystemStatus_State_e navState; // Enum (`navSystemStatus_State_e`): Current navigation state (`NAV_Status.state`)
    ...
    navSystemStatus_Error_e navError; // Enum (`navSystemStatus_Error_e`): Current navigation error code (`NAV_Status.error`)
    int16_t targetHeading; // Target heading for heading controller (`getHeadingHoldTarget()`)
...
typedef struct MSP_PACKED {
    ...
    batteryState_e batteryState; // Enum `batteryState_e` Current battery state (`getBatteryState()`, see `BATTERY_STATE_*`)
...
typedef struct MSP_PACKED {
    ...
    uint16_t sensorStatus; // Bitmask: Sensor status
    uint32_t activeModesLow; // Bitmask: First 32 active modes
    uint8_t profile; // Current config profile index
    uint16_t cpuLoad; // Average system load percentage (`averageSystemLoadPercent`)
    uint16_t armingFlags; // Bitmask: Flight controller arming flags (`armingFlags`). Note: Truncated to 16 bits
    uint8_t accCalibAxisFlags; // Bitmask: Accelerometer calibrated axes flags (`accGetCalibrationAxisFlags()`)
...
typedef struct MSP_PACKED {
    ...
    hardwareSensorStatus_e gyroStatus; // Enum `hardwareSensorStatus_e` Gyro hardware status (`getHwGyroStatus()`)
    hardwareSensorStatus_e accStatus; // Enum `hardwareSensorStatus_e` Accelerometer hardware status (`getHwAccelerometerStatus()`)
    hardwareSensorStatus_e magStatus; // Enum `hardwareSensorStatus_e` Compass hardware status (`getHwCompassStatus()`)
    hardwareSensorStatus_e baroStatus; // Enum `hardwareSensorStatus_e` Barometer hardware status (`getHwBarometerStatus()`)
    hardwareSensorStatus_e gpsStatus; // Enum `hardwareSensorStatus_e` GPS hardware status (`getHwGPSStatus()`)
    hardwareSensorStatus_e rangefinderStatus; // Enum `hardwareSensorStatus_e` Rangefinder hardware status (`getHwRangefinderStatus()`)
    hardwareSensorStatus_e pitotStatus; // Enum `hardwareSensorStatus_e` Pitot hardware status (`getHwPitotmeterStatus()`)
    hardwareSensorStatus_e opflowStatus; // Enum `hardwareSensorStatus_e` Optical Flow hardware status (`getHwOpticalFlowStatus()`)
...
#define MSP_GPSSVINFO_REPLY_PAYLOAD_SIZE 5
...
// No request payload for MSP_OSD_VIDEO_CONFIG.

// No reply payload for MSP_OSD_VIDEO_CONFIG.

// MSP_SET_OSD_VIDEO_CONFIG
// No request payload for MSP_SET_OSD_VIDEO_CONFIG.

// No reply payload for MSP_SET_OSD_VIDEO_CONFIG.

// MSP_DISPLAYPORT
// No request payload for MSP_DISPLAYPORT.

// No reply payload for MSP_DISPLAYPORT.
...
typedef struct MSP_PACKED {
    uint8_t rssi; // RSSI value (0-255) provided by the external source; firmware scales it to 10-bit (`value << 2`)
...
typedef struct MSP_PACKED {
    rssiSource_e rssiSource; // Enum: Source of the RSSI value (`getRSSISource()`, see `rssiSource_e`)
...
typedef struct MSP_PACKED {
    gpsFixType_e fixType; // Enum `gpsFixType_e` GPS fix type
    uint8_t numSat; // Number of satellites
    int32_t latitude; // Latitude
    int32_t longitude; // Longitude
    uint16_t altitude; // Altitude in meters (converted to centimeters internally; limited to 0-65535 m)
    uint16_t speed; // Ground speed (`gpsSol.groundSpeed`)
} msp_MSP_SET_RAW_GPS_request_t;

#define MSP_SET_RAW_GPS_REQUEST_PAYLOAD_SIZE 14
...
typedef struct MSP_PACKED {
    ...
    gpsProvider_e gpsType; // Enum `gpsProvider_e` (Sets `gpsConfigMutable()->provider`)
    uint8_t legacyGpsBaud; // Ignored
    sbasMode_e gpsSbasMode; // Enum `sbasMode_e` (Sets `gpsConfigMutable()->sbasMode`)
...
typedef struct MSP_PACKED {
    ...
    navWaypointFlags_e flag; // Bitmask: Waypoint flags (`navWaypointFlags_e`)
...
typedef struct MSP_PACKED {
    uint16_t heading; // Target heading (0-359)
...
typedef struct MSP_PACKED {
    uint16_t[8] motorValues; // Array of motor values to set when disarmed. Only affects first `MAX_SUPPORTED_MOTORS` entries
...
typedef struct MSP_PACKED {
    ...
    inputSource_e inputSource; // Enum `inputSource_e` Input source for the mix (RC chan, Roll, Pitch...)
    int16_t rate; // Mixing rate/weight (`-1000` to `+1000`, percent with sign)
    uint8_t speed; // Speed/Slew rate limit (`0`=instant, higher slows response)
    uint8_t reserved1; // Always 0
...
typedef struct MSP_PACKED {
    ...
    inputSource_e inputSource; // Enum `inputSource_e` Input source for the mix
    int16_t rate; // Mixing rate/weight (`-1000` to `+1000`, percent with sign)
    uint8_t speed; // Speed/Slew rate limit (`0`=instant, higher slows response)
...
// MSP_RESERVE_1
// No request payload for MSP_RESERVE_1.

// No reply payload for MSP_RESERVE_1.

// MSP_RESERVE_2
// No request payload for MSP_RESERVE_2.

// No reply payload for MSP_RESERVE_2.
...
typedef struct MSP_PACKED {
    uint16_t[4] motorMix; // Weights for a single motor `[throttle, roll, pitch, yaw]`, each encoded as `(mix + 2.0) * 1000` (range 0-4000)
} msp_MSP2_COMMON_MOTOR_MIXER_reply_t;

// #define MSP2_COMMON_MOTOR_MIXER_REPLY_PAYLOAD_SIZE  /* variable or unknown size */
...
typedef struct MSP_PACKED {
    ...
    uint32_t functionMask; // Bitmask: enabled functions (`FUNCTION_*`)
...
typedef struct MSP_PACKED {
    serialPortIdentifier_e identifier; // Port identifier Enum (`serialPortIdentifier_e`)
    uint32_t functionMask; // Bitmask: functions to enable
...
typedef struct MSP_PACKED {
    ...
    uint16_t heading; // Heading of the POI
...
// No request payload for MSP2_COMMON_GET_RADAR_GPS.

typedef struct MSP_PACKED {
    int32_t poiLatitude; // Latitude of a radar POI
    int32_t poiLongitude; // Longitude of a radar POI
    int32_t poiAltitude; // Altitude of a radar POI
} msp_MSP2_COMMON_GET_RADAR_GPS_reply_t;

#define MSP2_COMMON_GET_RADAR_GPS_REPLY_PAYLOAD_SIZE 12
...
typedef struct MSP_PACKED {
    ...
    gpsFixType_e fixType; // Enum `gpsFixType_e` Type of GPS fix
...
typedef struct MSP_PACKED {
    ...
    uint32_t armingFlags; // Bitmask: Full 32-bit flight controller arming flags (`armingFlags`)
    boxBitmask_t activeModes; // Bitmask words for active flight modes (`packBoxModeFlags()`)
...
typedef struct MSP_PACKED {
    uint8_t batteryFlags; // Bitmask: Bit0=Full on plug-in, Bit1=Use capacity thresholds, Bits2-3=`batteryState_e` (`getBatteryState()`), Bits4-7=Cell count (`getBatteryCellCount()`)
...
typedef struct MSP_PACKED {
    ...
    uint8_t legacyGpsBaud; // Always 0 (Legacy)
    sbasMode_e gpsSbasMode; // Enum `sbasMode_e` GPS SBAS mode (`gpsConfig()->sbasMode`). 0 if `USE_GPS` disabled
    uint8_t rssiChannel; // RSSI channel index (1-based, 0 disables) (`rxConfig()->rssi_channel`)
    int16_t magDeclination; // Magnetic declination / 10 (`compassConfig()->mag_declination / 10`). 0 if `USE_MAG` disabled
    uint16_t vbatScale; // Voltage scale (`batteryMetersConfig()->voltage.scale`). 0 if `USE_ADC` disabled
    batVoltageSource_e vbatSource; // Enum `batVoltageSource_e` Voltage source (`batteryMetersConfig()->voltageSource`). 0 if `USE_ADC` disabled
    ...
    batCapacityUnit_e capacityUnit; // Enum `batCapacityUnit_e` Capacity unit (`batteryMetersConfig()->capacity_unit`)
...
typedef struct MSP_PACKED {
    ...
    gpsProvider_e gpsType; // Enum `gpsProvider_e` Sets `gpsConfigMutable()->provider` (if `USE_GPS`)
    uint8_t legacyGpsBaud; // Ignored
    sbasMode_e gpsSbasMode; // Enum `sbasMode_e` Sets `gpsConfigMutable()->sbasMode` (if `USE_GPS`)
    uint8_t rssiChannel; // Sets `rxConfigMutable()->rssi_channel` (1-based, 0 disables) when <= `MAX_SUPPORTED_RC_CHANNEL_COUNT`
    int16_t magDeclination; // Sets `compassConfigMutable()->mag_declination = value * 10` (if `USE_MAG`)
    uint16_t vbatScale; // Sets `batteryMetersConfigMutable()->voltage.scale` (if `USE_ADC`)
    batVoltageSource_e vbatSource; // Enum `batVoltageSource_e` Sets `batteryMetersConfigMutable()->voltageSource` (if `USE_ADC`, validated)
    ...
    batCapacityUnit_e capacityUnit; // Enum `batCapacityUnit_e` Sets `batteryMetersConfigMutable()->capacity_unit` (validated, updates OSD energy unit if changed)
...
typedef struct MSP_PACKED {
    ...
    batVoltageSource_e vbatSource; // Enum `batVoltageSource_e` Voltage source (`batteryMetersConfig()->voltageSource`)
    ...
    int16_t currentOffset; // Current sensor offset (`batteryMetersConfig()->current.offset`)
    int16_t currentScale; // Current sensor scale (`batteryMetersConfig()->current.scale`)
    uint32_t capacityValue; // Battery capacity (`currentBatteryProfile->capacity.value`)
    uint32_t capacityWarning; // Capacity warning threshold (`currentBatteryProfile->capacity.warning`)
    uint32_t capacityCritical; // Capacity critical threshold (`currentBatteryProfile->capacity.critical`)
    batCapacityUnit_e capacityUnit; // Enum `batCapacityUnit_e` Capacity unit (`batteryMetersConfig()->capacity_unit`)
...
typedef struct MSP_PACKED {
    ...
    uint16_t vbatMaxCell; // Sets `currentBatteryProfileMutable->voltage.cellMax` (if `USE_ADC`)
    uint16_t vbatWarningCell; // Sets `currentBatteryProfileMutable->voltage.cellWarning` (if `USE_ADC`)
    int16_t currentOffset; // Sets `batteryMetersConfigMutable()->current.offset`
    int16_t currentScale; // Sets `batteryMetersConfigMutable()->current.scale`
    uint32_t capacityValue; // Sets `currentBatteryProfileMutable->capacity.value`
    uint32_t capacityWarning; // Sets `currentBatteryProfileMutable->capacity.warning`
    uint32_t capacityCritical; // Sets `currentBatteryProfileMutable->capacity.critical`
    batCapacityUnit_e capacityUnit; // Enum `batCapacityUnit_e` Sets `batteryMetersConfigMutable()->capacity_unit` (validated, updates OSD energy unit if changed)
...
typedef struct MSP_PACKED {
    uint32_t airspeed; // Estimated/measured airspeed (`getAirspeedEstimate()`, cm/s). 0 if unavailable
...
// No reply payload for MSP2_INAV_TIMER_OUTPUT_MODE.
...
typedef struct MSP_PACKED {
    ...
    outputMode_e outputMode; // Output mode override (`outputMode_e` enum) to set
...
// No reply payload for MSP2_INAV_OSD_LAYOUTS.
...
typedef struct MSP_PACKED {
    ...
    uint8_t itemIndex; // Index of the OSD item
    uint16_t itemPosition; // Packed X/Y position using `OSD_POS(x, y)` with `OSD_VISIBLE_FLAG` bit
...
typedef struct MSP_PACKED {
    ...
    uint8_t currentAlarm; // Current draw alarm threshold (`osdConfig()->current_alarm`)
    int16_t imuTempMinAlarm; // Min IMU temperature alarm (`osdConfig()->imu_temp_alarm_min`)
    int16_t imuTempMaxAlarm; // Max IMU temperature alarm (`osdConfig()->imu_temp_alarm_max`)
    int16_t baroTempMinAlarm; // Min Baro temperature alarm (`osdConfig()->baro_temp_alarm_min`). 0 if `USE_BARO` disabled
    int16_t baroTempMaxAlarm; // Max Baro temperature alarm (`osdConfig()->baro_temp_alarm_max`). 0 if `USE_BARO` disabled
...
typedef struct MSP_PACKED {
    ...
    int16_t imuTempMinAlarm; // Sets `osdConfigMutable()->imu_temp_alarm_min`
    int16_t imuTempMaxAlarm; // Sets `osdConfigMutable()->imu_temp_alarm_max`
    int16_t baroTempMinAlarm; // Sets `osdConfigMutable()->baro_temp_alarm_min` (if `USE_BARO`)
    int16_t baroTempMaxAlarm; // Sets `osdConfigMutable()->baro_temp_alarm_max` (if `USE_BARO`)
...
typedef struct MSP_PACKED {
    videoSystem_e videoSystem; // Enum `videoSystem_e`: Video system (Auto/PAL/NTSC) (`osdConfig()->video_system`)
    ...
    osd_crosshairs_style_e crosshairsStyle; // Enum `osd_crosshairs_style_e`: Style of the center crosshairs (`osdConfig()->crosshairs_style`)
    osd_sidebar_scroll_e leftSidebarScroll; // Enum `osd_sidebar_scroll_e`: Left sidebar scroll behavior (`osdConfig()->left_sidebar_scroll`)
    osd_sidebar_scroll_e rightSidebarScroll; // Enum `osd_sidebar_scroll_e`: Right sidebar scroll behavior (`osdConfig()->right_sidebar_scroll`)
    ...
    osd_stats_energy_unit_e statsEnergyUnit; // Enum `osd_stats_energy_unit_e`: Unit for energy display in post-flight stats (`osdConfig()->stats_energy_unit`)
...
typedef struct MSP_PACKED {
    videoSystem_e videoSystem; // Sets `osdConfigMutable()->video_system`
    ...
    osd_crosshairs_style_e crosshairsStyle; // Sets `osdConfigMutable()->crosshairs_style`
    osd_sidebar_scroll_e leftSidebarScroll; // Sets `osdConfigMutable()->left_sidebar_scroll`
    osd_sidebar_scroll_e rightSidebarScroll; // Sets `osdConfigMutable()->right_sidebar_scroll`
    uint8_t sidebarScrollArrows; // Sets `osdConfigMutable()->sidebar_scroll_arrows`
    osd_unit_e units; // Sets `osdConfigMutable()->units` (enum `osd_unit_e`)
    osd_stats_energy_unit_e statsEnergyUnit; // Sets `osdConfigMutable()->stats_energy_unit`
...
typedef struct MSP_PACKED {
    int32_t[DEBUG32_VALUE_COUNT] debugValues; // Values from the `debug` array (signed, typically 8 entries)
...
typedef struct MSP_PACKED {
    ...
    BlackboxDevice blackboxDevice; // Enum `BlackboxDevice`: Target device for logging (`blackboxConfig()->device`). 0 if not supported
...
typedef struct MSP_PACKED {
    BlackboxDevice blackboxDevice; // Sets `blackboxConfigMutable()->device`
...
typedef struct MSP_PACKED {
    ...
    int16_t alarmMin; // Min temperature alarm threshold (`sensorConfig->alarm_min`)
    int16_t alarmMax; // Max temperature alarm threshold (`sensorConfig->alarm_max`)
...
typedef struct MSP_PACKED {
    tempSensorType_e type; // Sets sensor type (`tempSensorType_e`)
    uint64_t address; // Sets sensor address/ID
    int16_t alarmMin; // Sets min alarm threshold (`tempSensorConfigMutable(index)->alarm_min`)
    int16_t alarmMax; // Sets max alarm threshold (`tempSensorConfigMutable(index)->alarm_max`)
...
typedef struct MSP_PACKED {
    ...
    uint8_t simulatorFlags_t; // Bitmask: Options for HITL (`HITL_*` flags)
    gpsFixType_e gpsFixType; // Enum `gpsFixType_e` Type of GPS fix (If `HITL_HAS_NEW_GPS_DATA`)
...
typedef struct MSP_PACKED {
    ...
    inputSource_e p2InputSource; // (Optional) Profile 2 Enum `inputSource_e` Input source
...
#define MSP2_INAV_SERVO_MIXER_REPLY_PAYLOAD_SIZE 12
...
typedef struct MSP_PACKED {
    ...
    logicOperation_e operation; // Enum `logicOperation_e` Logical operation (AND, OR, XOR, etc.)
    ...
    uint8_t flags; // Bitmask: Condition flags (`logicConditionFlags_e`)
...
typedef struct MSP_PACKED {
    ...
    int8_t activatorId; // Activator condition ID (-1/255 if none)
    logicOperation_e operation; // Enum `logicOperation_e` Logical operation
...
typedef struct MSP_PACKED {
    int32_t[MAX_LOGIC_CONDITIONS] conditionValues; // Array of current values for each logic condition (`logicConditionGetValue(i)`). 1 for true, 0 for false, or numerical value depending on operation
...
typedef struct MSP_PACKED {
    int32_t[MAX_GLOBAL_VARIABLES] gvarValues; // Array of current values for each global variable (`gvGet(i)`)
...
typedef struct MSP_PACKED {
    int32_t[MAX_PROGRAMMING_PID_COUNT] pidOutputs; // Array of current output values for each Programming PID (`programmingPidGetOutput(i)`, signed)
...
typedef struct MSP_PACKED {
    ...
    int32_t approachAlt; // Signed altitude for the approach phase (`navFwAutolandApproach_t.approachAlt`)
    int32_t landAlt; // Signed altitude for the final landing phase (`navFwAutolandApproach_t.landAlt`)
    fwAutolandApproachDirection_e approachDirection; // Enum `fwAutolandApproachDirection_e`: Direction of approach (From WP, Specific Heading)
...
typedef struct MSP_PACKED {
    uint8_t approachIndex; // Index of the approach setting (0 to `MAX_FW_LAND_APPOACH_SETTINGS - 1`)
    int32_t approachAlt; // Signed approach altitude (`navFwAutolandApproach_t.approachAlt`)
    int32_t landAlt; // Signed landing altitude (`navFwAutolandApproach_t.landAlt`)
    fwAutolandApproachDirection_e approachDirection; // Enum `fwAutolandApproachDirection_e` Sets approach direction
...
typedef struct MSP_PACKED {
    ...
    /* UNKNOWN_CTYPE */ uint8_t field; 
} msp_MSP2_ADSB_VEHICLE_LIST_reply_t;

// #define MSP2_ADSB_VEHICLE_LIST_REPLY_PAYLOAD_SIZE  /* variable or unknown size */
...
typedef struct MSP_PACKED {
    ...
    char elementText[]; // Static text bytes
} msp_MSP2_INAV_CUSTOM_OSD_ELEMENT_reply_t;

// #define MSP2_INAV_CUSTOM_OSD_ELEMENT_REPLY_PAYLOAD_SIZE  /* variable or unknown size */
...
typedef struct MSP_PACKED {
    ...
    osdCustomElementTypeVisibility_e visibilityType; // Visibility condition source
    uint16_t visibilityValue; // Value/ID of the visibility condition source
    char elementText[]; // Raw bytes
} msp_MSP2_INAV_SET_CUSTOM_OSD_ELEMENTS_request_t;

// #define MSP2_INAV_SET_CUSTOM_OSD_ELEMENTS_REQUEST_PAYLOAD_SIZE  /* variable or unknown size */
...
typedef struct MSP_PACKED {
    ...
    pinLabel_e pinLabel; // Label for special pin usage (`PIN_LABEL_*` enum, e.g., `PIN_LABEL_LED`). 0 (`PIN_LABEL_NONE`) otherwise
...
typedef struct MSP_PACKED {
    ...
    int16_t max; // Maximum servo endpoint (`servoParams(i)->max`)
    int16_t middle; // Middle/Neutral servo position (`servoParams(i)->middle`)
    int8_t rate; // Servo rate/scaling (`servoParams(i)->rate`)
...
typedef struct MSP_PACKED {
    uint8_t servoIndex; // Index of the servo to configure (0 to `MAX_SUPPORTED_SERVOS - 1`)
    int16_t min; // Sets minimum servo endpoint
    int16_t max; // Sets maximum servo endpoint
    int16_t middle; // Sets middle/neutral servo position
    int8_t rate; // Sets servo rate/scaling
...
typedef struct MSP_PACKED {
    ...
    int32_t minAltitude; // Minimum allowed altitude within the zone (`geoZonesConfig(idx)->minAltitude`)
    int32_t maxAltitude; // Maximum allowed altitude within the zone (`geoZonesConfig(idx)->maxAltitude`)
    uint8_t isSeaLevelRef; // Boolean: 1 if altitudes are relative to sea level, 0 if relative to home
    fenceAction_e fenceAction; // Enum (`fenceAction_e`): Action to take upon boundary violation
...
typedef struct MSP_PACKED {
    ...
    uint8_t type; // Define (`GEOZONE_TYPE_EXCLUSIVE/INCLUSIVE`): Zone type (Inclusion/Exclusion)
    uint8_t shape; // Define (`GEOZONE_SHAPE_CIRCULAR/POLYGON`): Zone shape (Polygon/Circular)
    int32_t minAltitude; // Minimum allowed altitude (`geoZonesConfigMutable()->minAltitude`)
    int32_t maxAltitude; // Maximum allowed altitude (`geoZonesConfigMutable()->maxAltitude`)
...
typedef struct MSP_PACKED {
    ...
    int32_t radius; // If vertex is circle, Radius of the circular zone
} msp_MSP2_INAV_GEOZONE_VERTEX_reply_t;

#define MSP2_INAV_GEOZONE_VERTEX_REPLY_PAYLOAD_SIZE 14
