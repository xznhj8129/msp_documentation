...
def slugify_gfm(heading_text: str) -> str:
    """
    Approximate GitHub-style anchor:
    - remove backticks
    - lowercase
    - remove punctuation except spaces and hyphens/underscores
    ...
    # Keep alnum, space, hyphen, underscore, and hex chars
    text = re.sub(r"[^\w\s\-]", "", text)   # drop punctuation like () / :
    text = re.sub(r"\s+", "-", text)        # spaces -> hyphens
    text = re.sub(r"-{2,}", "-", text)      # collapse --- to -
    return text.strip("-")
...
def build_index(json_by_code: Dict[int, str]) -> str:
    ...
    for code, name in sorted(json_by_code.items()):
        hex_str = hex(code)
        heading_text = f"`{name}` ({code} / {hex_str})"
        slug = slugify_gfm(heading_text)
        item = f"- [{name} ({code} / {hex_str})](#{slug})"
        if 0 <= code <= 255:
            v1.append(item)
...
def generate_markdown(defs: Dict[str, Any]) -> str:
    ...
    return "# MultiWii Serial Protocol (MSP) – Autogenerated Reference\n\n" + index_md + "\n" + "".join(sections)

def main():
    in_path = Path(sys.argv[1]) if len(sys.argv) >= 2 else Path("lib/msp_messages.json")
    out_path = Path(sys.argv[2]) if len(sys.argv) >= 3 else Path("MSP_Doc.md")
